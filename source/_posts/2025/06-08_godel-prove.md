...
哥德尔第一不完备性定理：如果⽪亚诺算术公理没有⽭盾，那么存在⼀个算术命题，它是真的，但不能通过算术公理证明出来。

哥德尔第二不完备性定理：如果⽪亚诺算术公理没有⽭盾，那么“算术公理没有⽭盾”这个命题本⾝不能通过算术公理证明出来。
...
由于算术命题 A 是任取的，这说明，对任意算术命题，总有通用的方法能在有限步内给出正确判断。但这与上文中证明的“不存在通用算法 V 能够判断任意一个算术命题是否为真”矛盾。

因此，原假设不成立，公理体系 T 要么不一致，要么不完备。

一般来说，我们不愿意承认 T 是不一致的（否则我们会证明出各种天马行空的结果，如 1=0 等等），所以只能承认 T 是不完备的。
